...
| El mensaje de error que ve el usuario **nunca** lleva ids internos (UUID), nombres de clases/constraints/tablas ni texto de excepciones del framework | Es incomprensible para quien usa el sistema y revela información interna. El detalle técnico (ids, valores, causa) va **solo al log** (`log.warn` en el Service, justo antes del `throw`); el mensaje del `throw` se escribe para el usuario: qué pasó y qué puede hacer. Fechas con `FormatoMensaje.fecha/fechaHora`, duraciones con `FormatoMensaje.duracion` y estados con `FormatoMensaje.estado` (en `Services/Utils`), nunca el `toString()` de Java. |
| El `GlobalExceptionHandler` cubre también los errores del framework: JSON ilegible / valor con formato inválido (400), parámetro de ruta o query inválido o ausente (400), verbo no soportado (405), Content-Type no soportado (415), ruta inexistente (404) y violación de integridad de BD (409) | Sin estos handlers caían en el genérico 500 "error inesperado", que no le dice nada al usuario. Cada uno traduce a español, nombra el campo en forma legible (`fechaInicioVigencia` → "fecha inicio vigencia") y deja el detalle del framework en el log. |
| Locale fijo `es-AR` (`spring.web.locale` + `locale-resolver: fixed`) | Los mensajes por defecto de Bean Validation salen en el idioma de la JVM; así no dependen del servidor. |
| Las vigencias de `AgendaMedico` y `MedicoPrestacion` son **`LocalDate`** (columnas `date`), con **ambos extremos inclusivos** (`fechaInicioVigencia <= fecha <= fechaFinVigencia`; en `MedicoPrestacion` un fin vacío = sin corte). "Hoy" es la fecha en la **zona horaria de la clínica** (`ClinicaDomainService.findFechaActualClinica()`), no la del servidor ni la del cliente | Una vigencia es de calendario, no un instante. Las constraints de no solapamiento usan `daterange(inicio, fin, '[]')`. `IndicacionPrestacion` y `UsuarioRol` mantienen `ZonedDateTime`. |
